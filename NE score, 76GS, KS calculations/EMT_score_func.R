...
cat("Calculating EMT Score by using 76 gene signatures ....\n")
  ...
  print(setdiff(EMTSignature[,2],genes))
...
KSScore = function(expMat){
  ...
  print(setdiff(EMTSignature[,1],genes))
...
xlsx.addEMTscores<-function(sheetName,GS76, KS, MLR, correlationVal){
  EMTDF = data.frame("Sample_name" = GS76[,1],"KS" = KS[,1], "GS76" = GS76[,2], "MLR" = MLR[,2])
  write_xlsx(EMTDF, "../Output/all_EMT_Scores.xlsx",col_names = TRUE)
